...
var startCell = [0, 0];
var newGrid
var imgT1B1 = new Image();
var imgT2B2 = new Image();
var imgCross = new Image();
var imgBT2 = new Image();
var imgBT1 = new Image();
var imgTB2 = new Image();
var imgTB1 = new Image();
var imgT0B1 = new Image();
var imgT0B2 = new Image();
var imgT1B0 = new Image();
var imgT1B1F = new Image();
var imgT1B2 = new Image();
var imgT1B2F = new Image();
var imgT2B0 = new Image();
var imgT2B1 = new Image();
var imgT2B1F = new Image();
var imgT2B2F = new Image();
var helpMenu = false;
var render = true;
var del = false;
var forceOn = false;
...
var nextAt;
var ticks = 0;
var culling = { tick: [], tick2: [], connection: { horizontal: [], vertical: [], applicital: (default2Cell ? Array(gridWidth * gridHeight).fill(0).map((e, i) => { return [Math.floor(i / gridWidth), i % gridWidth] }) : []) } }
  ...
  horizontal: Array(gridWidth)
    ...
    .map(() =>
      Array(gridHeight)
        .fill(null)
        .map(() => {
          return { type: { upperType: 0, lowerType: 0 }, flipped: false };
        })
    ),
...
imgCross.src = "data:image/png;base64,"
...
function isConnected(target, shift = [0, 0]) {
  ...
  switch (cellGrid[tx][ty].type) {
    case 1:
      let connected = false
      connected = connected||(((cellConnections.horizontal[tx][ty].type.upperType != 0) || (cellConnections.horizontal[tx][ty].type.lowerType != 0)) && (cellConnections.horizontal[tx][ty].flipped))
      connected = connected||(((cellConnections.vertical[tx][ty].type.upperType != 0) || (cellConnections.vertical[tx][ty].type.lowerType != 0)) && (cellConnections.vertical[tx][ty].flipped))
      if (tx > 0) {
        connected = connected||(((cellConnections.horizontal[tx - 1][ty].type.upperType != 0) || (cellConnections.horizontal[tx - 1][ty].type.lowerType != 0)) && (!cellConnections.horizontal[tx - 1][ty].flipped))
      }
      if (ty > 0) {
        connected = connected||(((cellConnections.vertical[tx][ty - 1].type.upperType != 0) || (cellConnections.vertical[tx][ty - 1].type.lowerType != 0)) && (!cellConnections.vertical[tx][ty - 1].flipped))
      }
    ...
    case 2:
      ...
      let lowerConnected = lowerConnectedfalse
      upperConnected = upperConnected||((cellConnections.horizontal[tx][ty].type.upperType != 0) && (cellConnections.horizontal[tx][ty].flipped))
      lowerConnected = lowerConnected||((cellConnections.horizontal[tx][ty].type.lowerType != 0) && (cellConnections.horizontal[tx][ty].flipped))
      upperConnected = upperConnected||((cellConnections.vertical[tx][ty].type.upperType != 0) && (cellConnections.vertical[tx][ty].flipped))
      lowerConnected = lowerConnected||((cellConnections.vertical[tx][ty].type.lowerType != 0) && (cellConnections.vertical[tx][ty].flipped))
      if (tx > 0) {
        upperConnected = upperConnected||((cellConnections.horizontal[tx - 1][ty].type.upperType != 0) && (!cellConnections.horizontal[tx - 1][ty].flipped))
        lowerConnected = lowerConnected||((cellConnections.horizontal[tx - 1][ty].type.lowerType != 0) && (!cellConnections.horizontal[tx - 1][ty].flipped))
      }
      if (ty > 0) {
        upperConnected = upperConnected||((cellConnections.vertical[tx][ty - 1].type.upperType != 0) && (!cellConnections.vertical[tx][ty - 1].flipped))
        lowerConnected = lowerConnected||((cellConnections.vertical[tx][ty - 1].type.lowerType != 0) && (!cellConnections.vertical[tx][ty - 1].flipped))
      }
      upperConnected = upperConnected||((cellConnections.applicital[tx][ty].type != 3) && (cellConnections.applicital[tx][ty].flipped))
      lowerConnected = lowerConnected||((cellConnections.applicital[tx][ty].type != 3) && (!cellConnections.applicital[tx][ty].flipped))
      culling.tick2 = culling.tick2.filter((i) => { return (JSON.stringify(i) !== JSON.stringify([tx, ty, 0])) && (JSON.stringify(i) !== JSON.stringify([tx, ty, 1])) })
      culling.tick = culling.tick.filter((i) => { return JSON.stringify(i) !== JSON.stringify([tx, ty]) })
      if (upperConnected) {
...
function update() {
      ...
      if (currentCell[0] != lastCell[0] || currentCell[1] != lastCell[1]) {
        ...
        if ((Math.abs(direction[0]) === 1 && direction[1] === 0) && ((currentCell[0] - (direction[0] === 1 ? 1 : 0) >= 0) && (currentCell[0] - (direction[0] === 1 ? 1 : 0) < gridWidth - 1) && (currentCell[1] >= 0) && (currentCell[1] < gridHeight))) {
          ...
          if (shift) {
            ...
            cellConnections.horizontal[
              currentCell[0] - (direction[0] === 1 ? 1 : 0)
            ][currentCell[1]].type.upperType = (e.buttons === 4 || del ? 0 : e.buttons) * (mix ? -1 : 1);
          }
          else {
            if (!alt) {
              cellConnections.horizontal[
            ...
            cellConnections.horizontal[
              currentCell[0] - (direction[0] === 1 ? 1 : 0)
            ][currentCell[1]].type.lowerType = (e.buttons === 4 || del ? 0 : e.buttons) * (mix ? -1 : 1);
          }
          cellConnections.horizontal[
            currentCell[0] - (direction[0] === 1 ? 1 : 0)
  ...
  culling.connection.horizontal.forEach((item) => {
    let x2 = item[0];
    let y2 = item[1];
    if ((cellConnections.horizontal[x2][y2].type.upperType != 0) || (cellConnections.horizontal[x2][y2].lowerType != 0)) {
      drawConnection(
        [x2, y2],
  ...
  culling.connection.vertical.forEach((item) => {
    let x2 = item[0];
    let y2 = item[1];
    if ((cellConnections.vertical[x2][y2].type.upperType != 0) || (cellConnections.vertical[x2][y2].type.lowerType != 0)) {
      drawConnection(
        [x2, y2],
  ...
  culling.connection.applicital.forEach((item) => {
    let x2 = item[0];
    let y2 = item[1];
    if (cellConnections.applicital[x2][y2].type != 0) {
      drawConnection(
        [x2, y2],
    ...
    culling.tick2.forEach((item) => {
      if (!auto || !paused) {
        ...
        switch (item[2]) {
          case 0:
            if (x2 > 0) {
              let mixed = ((Math.sign(cellConnections.horizontal[x2 - 1][y2].type.lowerType) == -1) || (Math.sign(cellConnections.horizontal[x2 - 1][y2].type.upperType) == -1))
              switch (cellGrid[x2 - 1][y2].type) {
                case 1:
                  bit |=
